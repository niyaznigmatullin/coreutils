...
tests () {
    ...
    command="'cd ~/coreutils && timeout --preserve-status --verbose -k 1m 10m cargo test --features feat_os_unix_android --no-fail-fast test_pr:: >/sdcard/tests.log 2>&1; echo \$? >$probe'"
    run_termux_command "$command" "$probe"
    return_code=$?
